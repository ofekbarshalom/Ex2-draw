...
class Polygon_2DTest {
    ...
    void contains() {
        ...
        boolean actual = vertex1.contains(p1); // expected true
        assertTrue(actual);

        Point_2D p2 = new Point_2D(9,10);
        boolean actual2 = vertex1.contains(p2); // expected false
        assertFalse(actual2);
    }
    ...
    void copy() {
        ...
        assertArrayEquals(vertex1.getAllPoints(),vertex1Copy.getAllPoints());
    }
    ...
    void scale() {
        ...
        vertex1.scale(p1,-1);    // Scaled the polygon by -1 to return to the original size
        vertex1.scale(p1,-1);
        Point_2D[] expectedPoints2 = {p1,p2,p3,p4,p5};
        Point_2D[] actualPoints2 = vertex1.getAllPoints();
        assertArrayEquals(expectedPoints2,actualPoints2);

        vertex1.scale(p1,1);
        Point_2D[] expectedPoints3 = {p1,p2,p3,p4,p5};
        Point_2D[] actualPoints3 = vertex1.getAllPoints();
        assertArrayEquals(expectedPoints3,actualPoints3);
    }
    ...
    void rotate() {
        ...
        for (int i = 0; i < actualPoints.length; i++)
            assertTrue(expectedPoints[i].close2equals(actualPoints[i], Ex2_Const.EPS));
    }
}